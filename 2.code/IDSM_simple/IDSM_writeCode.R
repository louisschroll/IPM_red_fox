...
IDSM_writeCode <- function() {
  IDSM.code <- nimble::nimbleCode({
    ...
    for (j in 1:n_sites) {
      ...
      Density[1, j, 1] <- sum(Density[2:age_max, j, 1]) / 2 * R_year[1]
      ...
      N_exp[1:age_max, j, 1] <- Density[1:age_max, 1] * L[j, 1] * W * 2
    }
    ...
    for (j in 1:n_sites) {
      for (t in 2:n_years) {
        # Age class 0 (index = 1): reproduction
        Density[1, j, t] <- sum(Density[2:age_max, j, t]) / 2 * R_year[t]
        
        # Age classes 1 to 3 (indeces = 2, 3, 4): age classes 0, 1, and 2 survivors 
        for(a in 2:(age_max-1)){
          Density[a, j, t] <- Density[a - 1, j, t - 1] * S[t - 1]
        }		
        
        # Age class 4+ (index = age_max = 5): age class 4 and 5+ survivors
        Density[age_max, j, t] <- (Density[age_max-1, j, t-1] + Density[age_max, j, t-1]) * S[t-1]
        
        ## Adult and juvenile numbers
        N_exp[1:age_max, j, t] <- Density[1:age_max, j, t] * L[j, t] * W * 2
      }
    }
    ...
    for (t in 1:n_years) {
      N_tot_exp[t] <- sum(N_exp[1:age_max, 1:n_sites, t])
    }
    ...
    for(t in 1:n_years){
      for(a in 1:age_max){
        N_area[a, t] <- meanDens[a, t] * size_hunting_area
        C[a, t] ~ dbin(harvest_rate, N_area[a, t]) 
      }
    }
  })
  return(IDSM.code)
